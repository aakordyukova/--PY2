...
class LightsGarland:
    def __init__(self, color: str, length: int, number_of_modes: int, current_mode_number: int):
        """
                ...
                Пример:
                >>> garland = LightsGarland('multicolored', 3, 5, 1)  # инициализация экземпляра класса
        """
    ...
    def change_mode(self, mode_number: int) -> None:
        ...
        Пример:
        >>> garland = LightsGarland("white", 1, 7, 2)
...
class MusicCompactDisk:
    ...
    def __init__(self, cd_name: str, artist_name: str, song_list: list[str]):
        """
        Инициализация экземпляра класса.

        :param cd_name: Название диска
        :param artist_name: Исполнитель
        :param song_list: Список названий песен
        """
        ...
        if not isinstance(song_list, list):
            raise TypeError("Список песен должен быть описан переменной типа list")
        if not all(isinstance(song, str) for song in song_list):
            raise ValueError("Названия песен должны быть представленны переменными типа str")
    ...
    def is_song_in(self, song_name: str) -> bool:
        ...
        Пример:
        >>> cd1 = MusicCompactDisk("HEAL", "The Rose", ["~", "Definition of ugly is", "Childhood", "Shift", "Cure","See-Saw", "Sour"])
        >>> cd1.is_song_in('Shift') #True
        """
